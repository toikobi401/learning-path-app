...
export default async function DashboardPage() {
  ...
  const [activeGoals, completedLogs, thisWeekCount, recentActivity] = await Promise.all([
      ...
      select: {
        ...
        topic: {
          select: {
            ...
            phase: {
              select: {
                path: { select: { goal: { select: { id: true, title: true } } } },
              },
            },
          },
        },
    ...
    <div>
      ...
      {activeGoals === 0 && completedTopics === 0 && (
        <div className="rounded-lg border border-dashed border-gray-200 bg-white px-8 py-16 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-gray-500 dark:text-gray-400">
            Create your first goal and PathAI will generate a structured,
            week-by-week curriculum tailored to your schedule.
          </p>
          <Link
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Topics completed", value: completedTopics },
          { label: "This week", value: thisWeekCount, suffix: thisWeekCount === 1 ? " topic" : " topics" },
          { label: "Day streak", value: streak, suffix: streak === 1 ? " day" : " days" },
        ].map((stat) => (
          <div
            key={stat.label}
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-xs text-gray-400 dark:text-gray-500">
                    {log.topic.phase.path.goal.title}
                  </p>
                </div>
                <div className="ml-4 shrink-0 text-right">
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    {log.topic.estimated_hrs}h
                  </p>
      ...
      {(activeGoals > 0 || completedTopics > 0) && (
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Link
            ...
            className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white px-5 py-4 transition hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:hover:bg-gray-800"
          >
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">Weekly Review</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">AI analysis of your progress</p>
            </div>
          </Link>
          <Link
            href="/dashboard/chat"
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">Chat Assistant</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">Ask questions about your goal</p>
            </div>
          </Link>
        </div>
      )}
