...
export default function GoalsPage() {
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/goals/${editingGoal.id}`, {
      ...
      body: JSON.stringify({
        ...editForm,
        hours_per_day: parseFloat(editForm.hours_per_day),
      }),
    });
  ...
  return (
    <>
      ...
      <div className="mb-8 flex items-center justify-between border-b border-gray-200 pb-6 dark:border-gray-800">
        <div>
          <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Goals</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Track what you want to learn and generate a personalized path.
          </p>
        </div>
        <Link
          href="/dashboard/goals/new"
          className="rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gray-700 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-300"
        >
          + New goal
        </Link>
      </div>
      ...
      {goals.length === 0 && (
        <div className="rounded-lg border border-dashed border-gray-200 bg-white px-8 py-16 text-center dark:border-gray-700 dark:bg-gray-900">
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">No goals yet</h2>
          <p className="mx-auto mt-2 max-w-sm text-sm text-gray-500 dark:text-gray-400">
            Create your first goal and PathAI will generate a structured, week-by-week curriculum
            tailored to your schedule.
          </p>
          <Link
            href="/dashboard/goals/new"
            className="mt-6 inline-block rounded-md bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-gray-700 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-300"
          >
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 capitalize ${STATUS_COLOR[goal.status]}`}>
                    {goal.status}
                  </span>
                </div>
                <div className="flex shrink-0 gap-1">
                  <button
                    ...
                    className="rounded px-2 py-1 text-xs text-gray-400 transition hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-gray-800 dark:hover:text-gray-300"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => setDeletingId(goal.id)}
                    className="rounded px-2 py-1 text-xs text-red-400 transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950"
                  >
                    Delete
                  </button>
                </div>
              ...
              {/* Title */}
              <h2 className="mt-3 text-sm font-semibold text-gray-900 dark:text-gray-100 line-clamp-1">
                {goal.title}
              </h2>

              {/* Meta */}
              <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">
                {goal.hours_per_day}h/day &middot; Due {formatDeadline(goal.deadline)}
              </p>

              {/* Description */}
              <p className="mt-2 flex-1 text-sm text-gray-500 line-clamp-2 dark:text-gray-400">
                {goal.description}
              </p>
      ...
      {editingGoal && editForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-800 dark:bg-gray-900">
            <h2 className="mb-4 text-base font-semibold text-gray-900 dark:text-gray-100">Edit goal</h2>
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  minLength={3}
                  className="mt-1 block w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-100 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:focus:border-gray-600 dark:focus:ring-gray-800"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditForm({ ...editForm, level: e.target.value as Goal["level"] })}
                    className="mt-1 block w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-gray-400 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100"
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Hours/day</label>
                  <input
                    type="number"
                    value={editForm.hours_per_day}
                    onChange={(e) => setEditForm({ ...editForm, hours_per_day: e.target.value })}
                    min={0.5}
                    max={12}
                    step={0.5}
                    required
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-xl border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-800 dark:bg-gray-900">
            <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Delete goal?</h2>
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              This will permanently delete the goal and its learning path. This action cannot be undone.
            </p>
